...
const FilterSlider:React.FC<FilterSliderProps> = ({label, onChange, info}) => {
    ...
    return(
        runtime? <>
            ...
            <input id="filter-range" className="range" type='range' min={'25'} max={'233'} defaultValue={info as string} onChange={onChange} />
        </> :
        <>
...
const Filters: React.FC<FiltersProps> = ({setOpen}) => {
    ...
    const selects = [
        ...
        {label: "Sub-Genre:", onChange: (e:any) => changeFilter(e, 'subGenres'), options: subGenreOptions, value: selectedFilters.subGenres, multi: true, search: false, info: 'Ex: Heist, Dark Comedy, etc.'},
